...
namespace WindowsFormsApplication2
{
    class StandTest
    {
        ...
        {
            string sqlquary = @"SELECT TOP 6 日期, 当日平均进尺数, 上部平均数, 中部平均数, 下部平均数
                                FROM 支架报表数据表  ORDER BY 日期 DESC ";
        ...
        private void InsertToDB(double[] average)
        {
            ...
            string str = string.Format(@"IF NOT EXISTS(SELECT 1 
                                         ...
                                         ava, Math.Round(average[0], 2), Math.Round(average[1], 2), Math.Round(average[2], 2));
        ...
        private void Calculate()
        {
            for (int i = 0; i < locations.Length; i++)
            {
                ...
                string sqlquary = string.Format(@"SELECT CQBH, ZDATETIME, ZBH, WZ, P1
                                                                  ...
                                                                  AND WZ='{2}'", yestoday, today, locations[i]);
