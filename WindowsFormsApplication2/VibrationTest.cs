...
namespace WindowsFormsApplication2
{
    class VibrationTest
    {
        ...
        //添加图片
        public void AddPictures(string filePath,int flag)
        {
            //添加图片
            if (flag == 1 && filePath != null)
            {
                //左行左列有行右列 + 文件
                workSheet_excel.Pictures.Add(5, 2, 16, 11, filePath);
                SaveReportFile();
            }
            if (flag == 2 && filePath != null)
            {
                //L5 T12
                workSheet_excel.Pictures.Add(5, 12, 12, 20, filePath);
                SaveReportFile();
            }
        ...
        {
            string FilePath = @"..\..\modelFile\微震模板.xlsx";
            ...
            if (workBook_excel.Worksheets["Sheet5"] != null )
                workSheet_excel = workBook_excel.Worksheets["Sheet5"];
            else
            {
                workBook_excel.Worksheets.Add("Sheet5");
                workSheet_excel = workBook_excel.Worksheets["Sheet5"];
            }
        ...
        public void GetDataTable()
        {
            ...
            string excel_to_datatable_file_path = @"..\..\modelFile\微震报表.xlsx";//C:\Users\14439\Desktop\yingpanhao C:\Users\14439\Desktop
            //微震表的数据excel
            Workbook wb;
            wb = new Workbook(excel_to_datatable_file_path);
        ...
        private void CalculateAll(DataTable datatable)
        {
            ...
            //dataView.Sort = column[5] + " DESC";//按照第六列数字大小排序,从大到小
            //DataTable datatable_sort = dataView.ToTable();//转换成新的排好序的数据表
            //每一行存储到dataRow数组里 已经按照从大到小的顺序存储 最大的都在第一行！！！
            DataRow[] dataRows = datatable.Select(column[9] + " LIKE '%面%'",column[5] + " DESC");//

            //找到最大能量 算出震级 找到相应备注的位置
            string s = dataRows[0][column[5]].ToString();
            max_energy = double.Parse(dataRows[0][column[5]].ToString());
            quake_level = (Math.Log10(max_energy) - 1.8) / 1.9;
            location = dataRows[0][column[9]].ToString();

            //算出 面前 和 面后 的总能量
            for (int i = 0; i < dataRow_front.Length; i++)
                sum_energy_front += double.Parse(dataRow_front[i][column[5]].ToString());
            for (int i = 0; i < dataRow_behind.Length; i++)
                sum_energy_behind += double.Parse(dataRow_behind[i][column[5]].ToString());

            for (int i = 0; i < dataRows.Length; i++)
            {
                if (double.Parse(dataRows[i][column[5]].ToString()) > 20000)
                {
                    sum_energy += double.Parse(dataRows[i][column[5]].ToString());
                    sum++;
                }
            }

            string time = dataRows[0][column[0]].ToString().Replace('/', '-').Substring(5)
                                ...
                                int sum)
        {
            ...
            //Q3 S3
            Cell cellItem_sum_energy_2 = workSheet_excel.Cells["S3"];
            cellItem_sum_energy_2.PutValue(sum_energy);
            Cell cellItem_sum = workSheet_excel.Cells["Q3"];
